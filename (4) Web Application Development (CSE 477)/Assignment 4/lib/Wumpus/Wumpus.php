...
class Wumpus {
    ...
    // works with move function
    private function moveActual($ndx) {
        $this->current = $this->rooms[$ndx];
        if ($this->current->contains(1)) {
            return self::EATEN;
        } else if ($this->current->contains(2)) {
            $this->carried = true;
            return self::moveActual(rand(1, self::NUM_ROOMS));
        } else if ($this->current->contains(3)) {
            return self::FELL;
        } else {
            return self::HAPPY;
        }
